...
Robot::Robot(ros::NodeHandle& nh) :
        ...
        transmitter_(nh),
        heartbeat_(),
        command_subscriber_("/platform/cmd_actuators", &Robot::UpdateActuators, this),
        throttle_percent_(0.0),
        steering_angle_(0.0),
        imu_(nh),
        encoder_publisher_(nh),
        drivetrain_(nh) {
    nh.subscribe(command_subscriber_);
...
void Robot::UpdateActuators(const magellan_core::ActuatorCommand& cmd_actuators) {
    throttle_percent_ = cmd_actuators.velocity / kMaxVelocity;
    steering_angle_ = cmd_actuators.steering_angle;
}
